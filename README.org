...
Starting from version ~2.5~, each annotation can contains an arbitrary number of replies that are also annotations. The results is a set of annotation arranged in a tree structure called: a "/thread/".

*** ~C-c C-t~ (function annotate-show-thread-at-point)
Show a buffer with the annotation thread for the annotation under cursor.

*** ~C-c C-r~ (function annotate-reply-to)
