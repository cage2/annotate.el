...
Starting from version ~2.5~, each annotation can contains an arbitrary number of replies that are also annotations. The results is a set of annotation arranged in a tree structure called: "/thread/".
